...
The data (shown by joining the points with lines) is shown in the following plots. We can see that temperatures are colder in winter and hotter in summer, as expected, although some places have more variability than others. Precipitations are in general less abundant in summer than during the rest of the year. However, there is also great variability on the quantity of precipitations around different stations.
...
The first step is to smooth both the temperatures and precipitations by doing a Fourier Basis expansion, since the data are periodic in the interval (0, 365) and fitting it to the data with roughness penalty, as done in the first assignment. In order to do this, Cross Validation can be used to find $K$ minimizing the sum of squares. In order to reduce computational costs, Generalized Cross Validation (GCV) is used. The problem with this method is that it tends to select a very high number of basis functions, thus basically interpolating the data and creating very rough (not smoothed) curves. The approach that solves this is penalized smoothing. The following plots illustrate the difference between the different (odd) number of basis functions.
...
```{r, results="hide"}
plot(smooth_temp_5,pch=20,col=color_1,xlab="day of the year",ylab="Temperature (C)")
title(main="Smoothing with OLS with 5 Fourier basis functions")
lines(smooth_temp_5,lty=1,lwd=3,col=color_1)

plot(smooth_temp_7,pch=20,col=color_1,xlab="day of the year",ylab="Temperature (C)")
title(main="Smoothing with OLS with 7 Fourier basis functions")
lines(smooth_temp_7,lty=1,lwd=3,col=color_1)


plot(smooth_temp_9,pch=20,col=color_1,xlab="day of the year",ylab="Temperature (C)")
title(main="Smoothing with OLS with 9 Fourier basis functions")
lines(smooth_temp_9,lty=1,lwd=3,col=color_1)

plot(smooth_temp_11,pch=20,col=color_1,xlab="day of the year",ylab="Temperature (C)")
title(main="Smoothing with OLS with 11 Fourier basis functions")
...
```{r, results="hide"}
plot(smooth_lprec_5,pch=20,col=color_1,xlab="day of the year",ylab="log precipitation (log mm)")
title(main="Smoothing with OLS with 5 Fourier basis functions")
lines(smooth_lprec_5,lty=1,lwd=3,col=color_1)

plot(smooth_lprec_7,pch=20,col=color_1,xlab="day of the year",ylab="log precipitation (log mm)")
title(main="Smoothing with OLS with 7 Fourier basis functions")
...
title(main="Smoothing with OLS with 9 Fourier basis functions")
lines(smooth_lprec_9,lty=1,lwd=3,col=color_1,xlab="day of the year",ylab="log precipitation (log mm)")

plot(smooth_lprec_11,pch=20,col=color_1,xlab="day of the year",ylab="log precipitation (log mm)")
title(main="Smoothing with OLS with 11 Fourier basis functions")
lines(smooth_lprec_11,lty=1,lwd=3,col=color_1)
```

The best smoothing is the one with 7 basis functions for temperature and 5 for the logarithm of the precipitation. Now we just need to find the penalization parameter by minimizing the value obtained with Generalized Cross Validation. 
...
The optimal values are 2250 for temperatures and 13250 for log precipitations. The final smoothed functions are shown in the following plots.
```{r, results="hide"}
...
Finally, since we are doing regression, it is interesting to check for outliers and remove them, if any. We can do this using depths, that are a measure of the centrality of the functions, as done in the second assignment.
...
boxplot(log_prec,col=color_1, main="Boxplot of log prec")
...
Next, we call the function `fRegress` of the `fda` package to carry out the fitting of the regression. We do this for the four values of $K$, but it was impossible to do for $k=5$ because the matrix was singular, so we ommit it in the following process.
...
First, we focus on the estimation of the intercept and the functional slope of the model. In the following plots one can see the four estimates of the functional slope of the model. We can see that the larger the value of $K$, the more ups and downs the estimate of the functional slope of the model and that the slope is in general different through the values of $K$:
...
On the other hand, we can have a look at the estimates of the intercept, which are close but different:
...
Next, we define the predicted values of the four fittings and compare their values. The main conclusion is that although the estimates of the functional slope $\beta$ are somehow different, the predicted values of the **average yearly of the base $10$ logarithm of precipitation** with the three different values are almost identical, with more than 98% correlation as can be seen in the following scatterplot and as the corrplot illustrates.
...
Also, from the predicted values, we obtain the residuals and the residual variance of the four fittings. We can see that the residual variance for $K=7$ is the smallest one but in general the results are close.
...
The predicted values also allow us to compute the **coefficient of determination** for the three values of $K$. For that we compute first the explained sums of squares for the three values of $K$ and the total sums of squares, and obtain the ratio between these quantities. The coefficients of determination obtained are $0.5620$, $0.5757$ and $0.5894$ respectively. Note the effect of $K$ in this estimation. The higher the $K$, the better, although the results are not very good. Therefore, we compute also the **adjusted coefficient of determination** that gives us values $0.4484$, $0.4229$, $0.3930$. We can see that the adjusted coefficient of determination for $K=7$ is the best one although not great. The main conclusion is that the daily temperatures don't explain **linearly** a large amount of annual precipitation, just around half of its variance.
...
Then, having all the previous into account, the question is which $K$ is the most appropriate? Apparently, $K=7$ is a good option but we need something more reliable like the GCV criterion. Then, we compute the value of the GCV for the four fittings. In this case, $K=7$ is the one with smaller GCV, so it seems that it is the best choice among the three values of $K$ considered:
...
We will see later than $2$ FPCs are able to explain more than the $95%$ of the total variability of the temperatures. Nevertheless, the idea is to compare the results when taking a number of FPCs from $1$ to $7$. For that, and for each value of $L$ ranging from $1$ to $7$, we proceed to estimate the coefficients $b_1,b_2,\ldots$ using the centered annual precipitations and the FPCSs and, subsequently, the estimate of the **functional slope of the model** $\beta$. Note that, to do all the previous, we are using that the FPCSs are uncorrelated. Therefore, the estimates of the coefficients $b_1,b_2,\ldots$ do no vary if we obtain the OLS for the different values of $L$. In other words, the estimate of $b_1$ is the same if we estimate the model only for $L=1$ or for $L=2$. Thus, we construct the estimate of the functional slope of the model recursively starting with the one for $L=1$ and then adding the new information given a new FPCSs. We also obtain estimates of the error variance and compute the value of the **GCV** and **BIC** for comparison of the different values of $L$.
...
Then, we have a look at the estimate of the functional slope `beta` with $L=6$. The results indicate that the estimate of $\beta$ with this method (blue) is almost the same as the one with the basis expansion (orange):
...
Finally in this section, let's see the residuals and the predictions with this method. These are compared with those with the basis expansion of $\beta$ and the two sets of predictions appear to be very similar. Also, we compute the residual variance and the $R^2$ and $R_{adj}^2$ that suggest that this fitting is better than the previous one, with an $R^2=0.56$ and an $R_{adj}^2=0.52$.
...
To carry out function-on-function regression, we need the smoothed **temperatures** (average daily temperature) and **precipitations** (base $10$ logarithm of the average daily precipitation). We smoothed the temperatures and precipitations, so we can just use the final ones without Navacerrada.
...
Here, we are going to obtain the **sample functional principal components** corresponding to the smoothed temperatures and precipitations. These are necessary to carry out the estimation of the model parameters in the function-on-function regression. For that, we will use the function `pca.fd` of the `fda` package. In particular, we take `nharm` is the number of FPCs that we want to obtain as $7$ for temperatures and 5 for logarithm of precipitations, that is the maximum allowed because we have taken $7$ and 5 Fourier basis, respectively:
...
We start by having a look at the eigenvalues. For that we look at the proportion of variability explained by the FPCs:
...
We can see that the first $2$ FPCs in both cases are able to explain almost all the variability. In particular, the $99.4%$ and the $96.9%$ of the total variability. We make a plot of the first two eigenfunctions for both temperatures and precipitations that have very similar behaviors. The first FPC shows that, in both cases, the variability is larger in autumn and winter than in the summer. On the other hand, the second FPC shows, in both cases, a contrast between the winter and the summer temperatures/precipitations. Therefore, there are two main sources of variation in both temperatures and precipitations. On the one hand, the different variability among seasons and, on the other hand, the differences between winter and summer:
...
Next, we have a look at the functional principal component scores (FPCSs) associated with the $7, 5$ FPCs of both variables. We can see that the scores show the presence of groups and probably outliers (not detected by the depth procedure) in both cases.
...
The next step is to obtain predicted precipitations and associated functional residuals that will help us to check whether the fitting is appropriate or not. Therefore, we obtain the predicted precipitations. We compare both of them, and see that the linear model appears to predict reasonable well the precipitations except those that are more extreme, that might as well be the potential outliers that we saw in the principal components. Since those 4 are very tight together and not alone, that might explain why the depth procedure didn't flag them.
...
We will use again the final smoothing with 5 basis functions and penalized smoothing without the outlier at Navacerrada and try to find groups.
...
Next, we apply methods based on the coefficients of the temperatures from the Fourier basis. For that we define the matrix containing such coefficients and make a plot to show all of them.

```{r }
# Note that now, the weather stations are the objects placed in the rows and the basis functions are the variables placed in the columns:
X <- t(final_smooth_daily_lprec_5_pen$fd$coefs)
#X
pairs(X,pch=19,col=color_1,main="Coefficients of the basis expansions")
```

We are going to run four different unsupervised classification methods for the data matrix: 
...
First, we run k-means. For that, note that we need to select the number of groups, so that we proceed as in the Multivariate Analysis course and compare the values of the average silhouette. We need to load the `factoextra` package and fix the maximum number of groups as $4$ (one less than the number of basis functions). The plot suggests the presence of only $4$ clusters, although 2 and 3 are also feasible. Therefore, we try $G=4$ and $100$ initial solutions, and plot the resulting solution after comparison of the $100$ final solutions. The solution is not perfectly defined and some of the points are not so clear. Still, the value of the silhouette (0.45) is good.
...
We can also see the map of Spain with the resulting clustering solution. Apparently, the solution separates the weather stations into which could seem cold coast climate, hot coast climate, arid climate and mountains, but it is not so clear. 
...
Second, we run PAM. Again, we select the number of groups with the average silhouette. The plot suggests the presence of $2$ clusters as before. Therefore, we try $G=2$ and plot the resulting solution. The solution is different to the one obtained with k-means and the sihouette is a bit higher, which is better. We can see in the map of Spain that the solution divides into northern and southern stations, which seems logical, although some points are out.
...
Third, we run agglomerative hierarchical clustering with the average linkage. For that, we need to compute initially the distances between the coefficients. We choose the Manhattan distance that somehow takes into account the presence of potential outliers. Then, we run the procedure and plot the dendogram obtained that suggest the presence of two clusters as in the previous two cases. Therefore, we try $G=2$ and plot the resulting solution. The solution is very similar to the obtained with PAM dividing north and south.
...
maps::map('world',ylim=c(25,45),xlim=c(-19,5))
...
fat=tecator$y$Fat
R=rep(1, times=length(fat)) # 1 high fat
R[fat<=20]=2 # 2 low fat

absorbances=tecator$absorp.fdata$data
absorbances_rangeval=tecator$absorp.fdata$rangeval
absorbances_argvals=tecator$absorp.fdata$argvals
...
The next step is to create a single matrix containing all boys and girls and a vector of factors that indicates whether the row in the matrix contains hogh or low fat. As we are going to use the functions in the `fda.usc` package, the matrix will have dimensions $215\times 100$, corresponding to $215$ wavelengths and $100$ observation points. As we are going to use these numbers frequently, we define $n=215$ and $J=100$.
...
In this case, we do not smooth the data now because the functions from the `fda.usc` package that we are going to use, does not require an external smoothing.

## Bootstrap methodology

In this section, we develop supervised classification using the bootstrap TER. For that, we are going to follow the next steps:

1. Generate uniformly at random $B$ training and test samples, where $B$ is determined later. The training sample will consist of $150$ heights (around the $70%$ of the sample pairs) and the test sample will consist of $65$ heights (around the $30%$ of the sample pairs). 

2. Run the classifier for each set of training and test samples. This will give us a set of predictions of the observations in the test samples.

3. With each set of predictions given by the classifier, we will obtain the TER, leading to $B$ values $TER_1,\ldots,TER_B$.

4. Obtain the bootstrap TER (BTER) as the sample mean of $TER_1,\ldots,TER_B$.

The supervised classification methods considered are:

1. Standard methods on the FPC scores: Linear Discriminant Analysis (LDA), Quadratic Discriminant Analysis (QDA), Naïve Bayes, Neural networks, Classification trees, Random forest and Support vector machines.

2. Functional logistic regression.

3. Functional Generalized Additive Models (FGAMs).

4. Functional k nearest neighbors.

5. Functional non-parametric discrimination.

6. Functional maximal depth classification.

7. Functional depth-depth classification:

The next code implements everything. As it is long, we summarize the steps to take and the objects that are defined. We fix the seed in order to replicate the results:

1. `B` is the number of bootstrap samples. The method based on maximal depth is very time consuming. Therefore, we restrict the number of bootstrap samples to $B=100$.

2. `n_train` is the number of pairs in the training samples.

3. `n_test` is the number of pairs in the test samples.

4. `lda_bter`, `qda_bter`, `naiveBayes_bter`, `nnet_bter`, `rpart_bter`, `randomForest_bter`, `svm_bter`, `glm_bter`, `gsam_bter`,  `knn_bter`, `np_bter`, `depth_bter` and `DD_bter`.
are the vectors that contains the value of the TER for each method and bootstrap sample.

5. In the first part of the loop, we define the training and test samples.

6. In the second part of the loop, we define the training and test samples as `fdata` objects (the heights) and as data.frame objects (the gender).

7. In the third part of the loop, we create a list with the training sample and another with the test sample.

8. In the fourth part of the loop, we create the FPC basis based on a bspline basis. For simplicity, we take $K=L=10$ bsplines and FPCs.

9. In the last parts of the loop, all the classifiers are run and the returns are saved in the corresponding vector.
...
n_train <- 150
n_test <- 65
lda_bter <- qda_bter <- naiveBayes_bter <- nnet_bter <- rpart_bter <- vector(mode="numeric",length=B)
randomForest_bter <- svm_bter <- glm_bter <- gsam_bter <- vector(mode="numeric",length=B)
knn_bter <- np_bter <- depth_bter <- DD_bter <- vector(mode="numeric",length=B)
for (b in 1 : B){
  
  # Define the training and test samples
  
  train <- sample(1:215,150)
  X_train <- X[train,]
  r_train <- r[train]
  X_test <- X[-train,]
  r_test <- r[-train]
  
  # Define the training and test samples as fdata objets (the heights) 
  # and as data.frame objects (the gender)
  
  X_train_func <- fdata(X_train,argvals=absorbances_argvals,rangeval=absorbances_rangeval)
  X_test_func <- fdata(X_test,argvals=absorbances_argvals,rangeval=absorbances_rangeval)
  r_train_df <- data.frame(r_train)
  r_test_df <- data.frame(r_test)
  
  # Create a list with the training sample and another with the test sample
  
  train_pairs <- list("df"=r_train_df,"x"=X_train_func)
  test_fd <- list("x"=X_test_func)
  
  # Create the FPC basis
  
  basis <- create.bspline.basis(rangeval=absorbances_rangeval,nbasis=10)
  basis_x <- create.pc.basis(X_train_func,l=1:10,basis=basis)
  
  # Run LDA
  
  fda_class_lda <- classif.lda(r_train~x,data=train_pairs,basis.x=basis_x)
  pred_test <- predict(fda_class_lda,test_fd,type="class")
  lda_bter[b] <- 1 - sum(r_test==pred_test)/n_test
  
  # Run QDA
  
  fda_class_qda <- classif.qda(r_train~x,data=train_pairs,basis.x=basis_x)
  pred_test <- predict(fda_class_qda,test_fd,type="class")
  qda_bter[b] <- 1 - sum(r_test==pred_test)/n_test
  
  # Run Naïve Bayes
  
  fda_class_naiveBayes <- classif.naiveBayes(r_train~x,data=train_pairs,basis.x=basis_x)
  pred_test <- predict(fda_class_naiveBayes,test_fd,type="class")
  naiveBayes_bter[b] <- 1 - sum(r_test==pred_test)/n_test
  
  # Run neural networks
  
  fda_class_nnet <- classif.nnet(r_train~x,data=train_pairs,basis.x=basis_x)
  pred_test <- predict(fda_class_nnet,test_fd,type="class")
  nnet_bter[b] <- 1 - sum(r_test==pred_test)/n_test
  
  # Run classification trees
  
  fda_class_rpart <- classif.rpart(r_train~x,data=train_pairs,basis.x=basis_x)
  pred_test <- predict(fda_class_rpart,test_fd,type="class")
  rpart_bter[b] <- 1 - sum(r_test==pred_test)/n_test
  
  # Run random forest
  
  fda_class_randomForest <- classif.randomForest(r_train~x,data=train_pairs,basis.x=basis_x)
  pred_test <- predict(fda_class_randomForest,test_fd,type="class")
  randomForest_bter[b] <- 1 - sum(r_test==pred_test)/n_test

  # Run support vector machines
    
  fda_class_svm <- classif.svm(r_train~x,data=train_pairs,basis.x=basis_x)
  pred_test <- predict(fda_class_svm,test_fd,type="class")
  svm_bter[b] <- 1 - sum(r_test==pred_test)/n_test

  # Run logistic regression
  
  fda_class_glm <- classif.glm(r_train~x,data=train_pairs,basis.x=basis_x)
  pred_test <- predict(fda_class_glm,test_fd,type="class")
  glm_bter[b] <- 1 - sum(r_test==pred_test)/n_test

  # Run generalized additive models
  
  fda_class_gsam <- classif.gsam(r_train~s(x,k=3),data=train_pairs,basis.x=basis_x)
  pred_test <- predict(fda_class_gsam,test_fd,type="class")
  gsam_bter[b] <- 1 - sum(r_test==pred_test)/n_test
  
  # Run k nearest neighbors
  
  h <- seq(3,15,by=2)
  fda_class_knn <- classif.knn(r_train,X_train,h)
  pred_test <- predict(fda_class_knn,X_test,type="class")
  knn_bter[b] <- 1 - sum(r_test==pred_test)/n_test
  
  # Run non-parametric discrimination
  
  fda_class_np <- classif.np(r_train,X_train)
  pred_test <- predict(fda_class_np,X_test,type="class")
  np_bter[b] <- 1 - sum(r_test==pred_test)/n_test
  
  # Run maximal depth classification
  
  fda_class_depth <- classif.depth(r_train,X_train_func,X_test_func)
  depth_bter[b] <- 1 - sum(r_test==fda_class_depth$group.pred)/n_test
  
  # Run depth-depth classification
  
  fda_class_DD <- classif.DD(r_train,X_train_func,control=list(draw=FALSE))
  pred_test <- predict(fda_class_DD,test_fd,type="class")
  DD_bter[b] <- 1 - sum(r_test==pred_test)/n_test

}
```

Finally, we obtain the sample means of the TER for each model and bootstrap sample. We show the results in a table and a boxplot. Note that the best results are attained by LDA, SVM, kNN and NP. Note that these are the methods that appears to work better for this example, but other methods may be better for other data sets.

```{r, eval=FALSE}
color_1 <- "deepskyblue2"
bter_lda <- mean(lda_bter)
bter_qda <- mean(qda_bter)
bter_naiveBayes <- mean(naiveBayes_bter)
bter_nnet <- mean(nnet_bter)
bter_rpart <- mean(rpart_bter)
bter_randomForest <- mean(randomForest_bter)
bter_svm <- mean(svm_bter)
bter_glm <- mean(glm_bter)
bter_gsam <- mean(gsam_bter)
bter_knn <- mean(knn_bter)
bter_np <- mean(np_bter)
bter_depth <- mean(depth_bter)
bter_DD <- mean(DD_bter)
BTERS <- c(bter_lda,bter_qda,bter_naiveBayes,bter_nnet,bter_rpart,
           bter_randomForest,bter_svm,bter_glm,bter_gsam,bter_knn,
           bter_np,bter_depth,bter_DD)
names(BTERS) <- c("LDA","QDA","NB","NNs","CT","RF",
                  "SVM","LR","GAM","kNN","NP","MD","DD")
BTERS
ALL.BTERS <- cbind(lda_bter,qda_bter,naiveBayes_bter,nnet_bter,rpart_bter,
                   randomForest_bter,svm_bter,glm_bter,gsam_bter,knn_bter,
                   np_bter,depth_bter,DD_bter)
colnames(ALL.BTERS) <- c("LDA","QDA","NB","NNs","CT","RF",
                         "SVM","LR","GAM","kNN","NP","MD","DD")
boxplot(ALL.BTERS,col=color_1,main="Bootstrap TER")
```

We can see that the methods with the lowest Test Error Rate are LDA, QDA, neural networks, linear regression and generalize additive models, all of them with similar performance, notably better than the rest and with lower variance.


![ ](/Users/arpri/OneDrive/Escritorio/resultadosejercicio3Funcional.PNG)
