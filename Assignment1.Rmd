...
It can be seen that most of the children follow more or less the same values but some of them follow curves far away from the group, with the possibility of being outliers. Also, the curves are obviously not smooth since we are just joining lines so the next step is to smooth them.
...
Now we want to smooth the results just for the first boy. The way one smooths is by doing a basis expansion. I will use the Fourier basis since the data represent a cycle so they are expected to be periodic. Knowing the basis functions one can just fit them appropriately to the data. However, it is first necessary to know the number K of basis functions we want to use, because there are an infinite number of them. In order to do this, Cross Validation can be used to find K minimizing the sum of squares. In order to reduce computational costs, Generalized Cross Validation (GCV) is used. The problem with this method is that it tends to select a very high number of basis functions, thus basically interpolating the data (overfitting) or creating very rough (not smoothed) curves. The approach that solves this is penalized smoothing. This method penalizes overfitting (associated with high values of $K$) by looking at the total curvature of the curve, since the more complicated the curve, usually the more curvature, by a factor of lambda, that one needs to optimize as well.
...
smooth_hipboy1_11 <- smooth.basis(argvals=x,y=hipboy1,
                                       fdParobj=fourier_basis_5)
smooth_hipboy1_15 <- smooth.basis(argvals=x,y=hipboy1,
                                       fdParobj=fourier_basis_7)
smooth_hipboy1_19 <- smooth.basis(argvals=x,y=hipboy1,
...
plot(x,residuals(smooth_hipboy1_11),pch=20,col=color_1,
     main="Residuals with 5 basis functions",xlab="Cycle",ylab="Temperature")
plot(x,residuals(smooth_hipboy1_15),pch=20,col=color_1,
     main="Residuals with 7 basis functions",xlab="Cycle",ylab="Temperature")
plot(x,residuals(smooth_hipboy1_19),pch=20,col=color_1,
     main="Residuals with 9 basis functions",xlab="Cycle",ylab="Temperature")
```

Now, we focus on the selection of $K$. For the previous fittings, it is easy to check that forthe first boy **the larger K, the smaller the GCV**.
...
for (i in 1:l_lambdas){
  ...
  smooth_hipboy1_7_pen <- smooth.basis(argvals=x,y=hipboy1,
...
Then, we proceed to make the fit with such value of $\lambda$. See the corresponding plot of the functions for the $39$ children.
